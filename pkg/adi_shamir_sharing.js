 ...
 * @returns {any}
 */
export function split_key(secret) {
    const ptr0 = passStringToWasm0(secret, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.split_key(ptr0, len0);
    if (ret[2]) {
 ...
 * @returns {string}
 */
export function reconstruct_key(shares) {
    let deferred2_0;
    let deferred2_1;
    try {
        const ret = wasm.reconstruct_key(shares);
        var ptr1 = ret[0];
        var len1 = ret[1];
        if (ret[3]) {
            ptr1 = 0; len1 = 0;
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('adi_shamir_sharing_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
